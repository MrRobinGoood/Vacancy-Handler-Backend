import gspread
from backend.core.model import train_model, cv, save_model
from backend.core.text_handler import prepared_table


def save_list_corpus_train(list_corpus_train):
    with open('./backend/resources/list_corpus_train.txt', 'w', encoding='utf-8') as f:
        f.write('.'.join(list_corpus_train))


def save_list_labels_train(list_labels_train):
    with open('./backend/resources/list_labels_train.txt', 'w', encoding='utf-8') as f:
        f.write('.'.join(list_labels_train))


def get_list_corpus_train():
    with open('./backend/resources/list_corpus_train.txt', 'r', encoding='utf-8') as f:
...
gc = gspread.service_account(filename='./backend/key.json')
sh = gc.open("cpHh")
